...
const pushClient_1 = __nccwpck_require__(5619);
var ACTION;
(function (ACTION) {
    ACTION["PUSH"] = "push";
    ACTION["PULL"] = "pull";
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        switch (core.getInput('action')) {
            case ACTION.PUSH: {
                const pushClient = new pushClient_1.LokalisePushClient(args);
                yield pushClient.pushKeys();
                break;
            }
            default: {
...
PO.prototype.save = function (filename, callback) {
    __nccwpck_require__(5747).writeFile(filename, this.toString(), callback);
};
...
PO.load = function (filename, callback) {
    __nccwpck_require__(5747).readFile(filename, 'utf-8', function (err, data) {
        if (err) {
            return callback(err);
        }
...
__nccwpck_require__.d(__webpack_exports__, {
  "LokalisePushClient": () => (/* binding */ LokalisePushClient)
});
...
var node_api = __nccwpck_require__(5919);
...
;// CONCATENATED MODULE: ./src/loader/loader.ts
...
(function (FILE_FORMAT) {
    FILE_FORMAT["PO_FORMAT"] = "po";
    FILE_FORMAT["JSON_FORMAT"] = "json";
...
function loadFile(filePath) {
    return __awaiter(this, void 0, void 0, function* () {
        return new Promise((resolve, reject) => {
            (0,external_fs_.readFile)(filePath, 'utf-8', (err, data) => {
                if (err) {
                    reject(err);
                    return;
                }
 ...
 */
function parsePOMessages(file, filename) {
    const translationMessages = po_default().parse(file).items;
 ...
 */
function parseJsonMessages(file, filename) {
    const translatedMessages = JSON.parse(file);
    return Object.keys(translatedMessages).map((key) => {
        return {
            keyId: key,
...
function loadTranslationFile(filepath, filename, format) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!filename.endsWith(format)) {
            return [];
        }
        let extractedMessages = [];
        const fullPath = (0,external_path_.join)(filepath, filename);
        try {
            const fileAsString = yield loadFile(fullPath);
...
;// CONCATENATED MODULE: ./src/lokalise/baseClient.ts
var baseClient_awaiter = (undefined && undefined.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
...
class LokaliseClient {
    constructor(args) {
        ...
        this.lokaliseApi = new node_api.LokaliseApi({ apiKey: args.apiKey });
    }
    /**
     * Builds an in-memory map of all translation messages based on this.directory
     *
     * Assumes:
     ...
     */
    getLocalKeys() {
        return baseClient_awaiter(this, void 0, void 0, function* () {
            const localKeys = [];
            try {
                const filePath = this.getLanguageFilePath(this.sourceLanguage);
                const fileNames = yield (0,external_util_.promisify)(external_fs_.readdir)(filePath);
                for (const fileName of fileNames) {
                    const data = yield loadTranslationFile(filePath, fileName, this.format);
                    core.info(`Found ${Object.keys(data).length} keys in language file ${fileName}'`);
                    localKeys.push(...data);
                }
            }
            catch (error) {
    ...
    getRemoteKeys() {
        return baseClient_awaiter(this, void 0, void 0, function* () {
            return yield this.lokaliseApi.keys.list({
...
;// CONCATENATED MODULE: ./src/lokalise/types.ts
/**
 * Based on the locale codes used in the file paths.
 */
var COLOR_LANGUAGE_ISO_CODE;
(function (COLOR_LANGUAGE_ISO_CODE) {
    ...
    COLOR_LANGUAGE_ISO_CODE["HAITIAN_CREOLE"] = "ht";
    COLOR_LANGUAGE_ISO_CODE["KHMER"] = "km";
    ...
    COLOR_LANGUAGE_ISO_CODE["CHINESE_SIMPLIFIED"] = "zh-Hans";
    COLOR_LANGUAGE_ISO_CODE["CHINESE_TRADITIONAL"] = "zh-Hant";
})(COLOR_LANGUAGE_ISO_CODE || (COLOR_LANGUAGE_ISO_CODE = {}));
/**
 ...
 */
const COLOR_LANGUAGE_ISO_CODE_MAPPER = {
    [COLOR_LANGUAGE_ISO_CODE.ARABIC]: 'ar',
    [COLOR_LANGUAGE_ISO_CODE.ENGLISH]: 'en',
    [COLOR_LANGUAGE_ISO_CODE.SPANISH]: 'es',
    [COLOR_LANGUAGE_ISO_CODE.FILIPINO]: 'fil',
...
var LOKALISE_DEVICE_PLATFORM;
(function (LOKALISE_DEVICE_PLATFORM) {
    LOKALISE_DEVICE_PLATFORM["WEB"] = "web";
    LOKALISE_DEVICE_PLATFORM["IOS"] = "ios";
    LOKALISE_DEVICE_PLATFORM["ANDRIOD"] = "android";
    LOKALISE_DEVICE_PLATFORM["OTHER"] = "other";
})(LOKALISE_DEVICE_PLATFORM || (LOKALISE_DEVICE_PLATFORM = {}));

;// CONCATENATED MODULE: ./src/lokalise/pushClient.ts
var pushClient_awaiter = (undefined && undefined.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
...
class LokalisePushClient extends LokaliseClient {
    /**
     * Create new translation messages and archive old ones.
     */
    pushKeys() {
        return pushClient_awaiter(this, void 0, void 0, function* () {
            try {
                const { items: remoteKeys } = yield this.getRemoteKeys();
                const localKeys = yield this.getLocalKeys();
                ...
                if (keysToCreate.length > 0) {
                    ...
                    const { items, errors } = yield this.lokaliseApi.keys.create(keysToCreate, {
                        project_id: this.projectId,
                    });
                ...
                if (keysToArchive.length > 0) {
                    ...
                    const { items, errors } = yield this.lokaliseApi.keys.bulk_update(keysToArchive, {
                        project_id: this.projectId,
                    });
     ...
     */
    getKeysToCreate(localKeys, remoteKeys) {
        const remoteKeyIds = new Set(remoteKeys.map(key => key.key_name[this.platform]));
        const keysToCreate = localKeys
            .filter(x => !remoteKeyIds.has(x.keyId))
            .map(key => this.buildLokaliseCreateKeysRequest(key));
        return keysToCreate;
    }
    buildLokaliseCreateKeysRequest(key) {
        const { keyId, translation, filename } = key;
        return {
            key_name: keyId,
            translations: [
                {
                    language_iso: COLOR_LANGUAGE_ISO_CODE.ENGLISH,
                    translation,
                },
            ],
...
"use strict";
module.exports = JSON.parse('{"name":"lokalise-actions","version":"0.0.0","private":true,"description":"Push to and pull from Lokalise","main":"lib/main.js","scripts":{"build":"tsc","format":"prettier --write **/*.ts","format-check":"prettier --check **/*.ts","lint":"eslint src/**/*.ts","package":"ncc build --source-map --license licenses.txt","test":"jest","all":"npm run build && npm run format && npm run lint && npm run package && npm test"},"repository":{"type":"git","url":"git+https://github.com/color/lokalise-actions.git"},"keywords":["actions","node","setup"],"author":"","license":"MIT","dependencies":{"@actions/core":"^1.4.0","@lokalise/node-api":"^6.2.1","pofile":"^1.1.1"},"devDependencies":{"@types/jest":"^26.0.15","@types/node":"^14.14.9","@typescript-eslint/parser":"^4.8.1","@vercel/ncc":"^0.28.6","eslint":"^7.29.0","eslint-plugin-github":"^4.1.3","eslint-plugin-jest":"^24.1.3","jest":"^26.6.3","jest-circus":"^26.6.3","js-yaml":"^3.14.0","prettier":"2.2.1","ts-jest":"^26.5.6","typescript":"^4.1.3"}}');
